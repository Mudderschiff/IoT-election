...
During this election, the Tallier simulates the voting process by generating random ballots and encrypting them using the joint key established in the previous phase. In example in the Appendix \ref{lst:intra-election} the Tallier generates 10 ballots and encrypts them. We test the configuration with 10, 100 and 1000 ballots.
...
\section{Networ Traffic Analysis}
Network traffice between the Laptop and the ESP32 was captured using Wireshark (4.4.3) by listening to the access point interface of the laptop. The MTU size of the Access Point is 1500 Bytes. Both guardians where positioned in close proximity to the laptop.The traffic was narrowed down by applying a display filter to filter out all non mqtt traffic. In the scenarios 10, 100 and 1000 votes no packet loss occured.Payloads such as public keys backups, decryptionshare that are exceeding the MTU size are simply split into multiple TCP segments and Reassembled. 

\begin{comment}
    Large payloads (e.g. keys/backups) split into 1440-byte segments (Frames 29-32, 82-84). MTU size. To calculate the time 

    Reassembly Delay: The time taken for all segments of a fragmented packet to arrive at the receiver, allowing the original packet to be reconstructed.We simply take the last package and subtract the first package (e.g. Frame 34 - Frame 29)

    [3 Reassembled TCP Segments (2955 bytes): #29(1440), #33(1440), #34(75)]
    [Frame: 29, payload: 0-1439 (1440 bytes)]
    [Frame: 33, payload: 1440-2879 (1440 bytes)]
    [Frame: 34, payload: 2880-2954 (75 bytes)]
    [Segment count: 3]
    [Reassembled TCP length: 2955]
    [Reassembled TCP Data […]: 

\end{comment}


The capture focused on the communication between the guardians and the tallier related to the key ceremony and the decryption phase. Furthermore, all captures start with the publishing of the ceremony\_details and end with the publishing of the decryption\_share. 3 captures where taken. One capture where the number of ballots was set to 10, another where it was set to 100 and the third where it was set to 1000 votes.

\subsection{Analysis with 10 Ballots}
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{images/10_ballots.png}
    \caption{Network Traffic Analysis with 10 Ballots}
    \label{fig:10-ballots}
\end{figure}



\subsection{Analysis with 100 Ballots}
...
\begin{comment}
    The precision of the timestamps is in milliseconds.  \ac{RTT} was calculated based on the time difference between request and acknowlegement packets for specific message exchanges. A display filter 
